...
def canMove(start: Vector2, end: Vector2) -> (bool, str):  # bool=if piece can move, str=reason why piece cant move
    ...
    if end not in canGo(start, board):
        return False, "Invalid move."

    newBoard = copy.deepcopy(board)
    changePos(start, end, newBoard)

    if isCheck(turn, newBoard):
...
def canGo(start: Vector2, _board: [[str]], checkr=False) -> [Vector2]:
    ...
    if piece[1] == "P":
        ...
        if not checkr:
            ...
            check = False
            for i in pieces:
                if findValue(turn + "K", _board) in canGo(i, _board, True):
                    check = True
                    break
...
def convertCoordToV2(value: str) -> (Vector2, Vector2):
    arr = ["a", "b", "c", "d", "e", "f", "g", "h"]

    return Vector2(arr.index(value[0]), int(value[1]) - 1), Vector2(arr.index(value[3]), int(value[4]) - 1)


def convertV2ToCoord(start: Vector2, end: Vector2):
    arr = ["a", "b", "c", "d", "e", "f", "g", "h"]
    coord = arr[int(start.x)] + str(start.y+1) + "-" + arr[int(end.x)] + str(end.y+1)

    return coord


def isCheck(color=turn, _board=board):
    pieces = getPieces("W" if color == "B" else "B", _board)
    check = False
    for i in pieces:
        if findValue(turn + "K", _board) in canGo(i, _board):
            check = True
            break
